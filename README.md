...
`MQTT_URL` — обязательная переменная. У монитора нет брокера по умолчанию: если адрес не задан, приложение завершится при запуске с ошибкой `MQTT_URL is required`.
